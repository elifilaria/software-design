...
int numColorButtons = 0;
Shape* colorButtonForeground[100];
Shape* colorButtonBackground[100];
...
void loadColorButtons(const char* filename){
  ...
  for (int j = 0; j < numColorButtons; j++){
    ...
    g >> key;
    if (key[0] == 'C'){
      colorButtonBackground[j] = new Circle(g);
      g >> key;
      colorButtonForeground[j] = new Circle(g);
    }
    else if (key[0] == 'R'){
      colorButtonBackground[j] = new Rectangle(g);
      g >> key;
      colorButtonForeground[j] = new Rectangle(g);
    }
  }
  g.close();
}
